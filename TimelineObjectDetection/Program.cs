...
using WebApplicationImageRecognition.Models.LOL;
...
namespace TimelineObjectDetection
{
    class Program
    {
        ...
        {
            // Delete all previous files in D:\Downloads\CustomVisionResults
            Directory.GetFiles(@"D:\Downloads\CustomVisionResults").ToList().ForEach(f => File.Delete(f));

            var watch = System.Diagnostics.Stopwatch.StartNew();

            List<string> fileNames = GetPicturesFileNames();
            foreach (var fileName in fileNames)
            {
                byte[] preparedImage = PrepareImage(fileName);
                ResponseObject responseObject = CallCustomVisionAPI(preparedImage);
        ...
        private static byte[] PrepareImage(string fileName)
        {
            ...
            byte[] imageInBytes = (byte[])_imageConverter.ConvertTo(image, typeof(byte[]));
            return imageInBytes;
        }

        public static void TranslateAndDrawTextToPicture(ResponseObject responseObject)
        {
            Image image = null;
        ...
        public static List<string> GetPicturesFileNames()
        {
            List<string> fileNames = new List<string>();
            //fileNames.Add(@"D:\Downloads\attachments (1)\IMG-5559 - Copy.JPG");
            fileNames.AddRange(Directory.GetFiles(@"D:\Downloads\test2"));

            return fileNames;
        }

        public static ResponseObject CallCustomVisionAPI(byte[] image)
        {
